...
export function rectNumFn2(config) {
  ...
  for (let i = 0; i < scale.divisions; i++) {
    ...
    if (i === 0 || i === scale.divisions-1) {
      if (typeRect === 'enteros') {
        ...
        i === 0 && ctx.fillText(`${mainData.initValue}`, xPos, yPos + scale.length*1.1)
        i !== 0 && ctx.fillText(`${eval(mainData.initValue) + scale.value*(scale.divisions-1)}`, xPos, yPos + scale.length*1.1)
      } else {
        ctx.font = `${font.weight} ${font.size*3}px ${font.family}`
        i === 0 && ctx.fillText(`${mainData.initValue}`, xPos, yPos + scale.length*1.1)
        i !== 0 && ctx.fillText(`${eval(mainData.initValue)+1}`, xPos, yPos + scale.length*1.1)          
      }
    }
  }
  ctx.restore()
...
function mostrarValores(state, mainData) {
  ...
  const { valuesUnit, valuesSeparator } = chart.values
  const { pointsData, centerChartY } = mainData
  ctx.save()
  for (let i = 0; i < scale.divisions; i++) {
    let xPos = pointsData.initPoint + pointsData.segment*i
    ...
    if (i > 0 && i < scale.divisions - 1) {
      if (typeRect === 'enteros' || typeRect === 'enteros con decimales' || typeRect === 'decimal' || typeRect === 'centesimal') {
        ...
        if (typeRect === 'enteros') {
          ...
          valToShow = eval(mainData.initValue)+multiplyNumber*i
          ...
          valToShow = eval(mainData.initValue) + valuesSeparator + i*multiplyNumber
        } else if (typeRect === 'decimal') {
          valToShow = eval(mainData.initValue) + valuesSeparator + i
        } else {
          valToShow = eval(mainData.initValue) + valuesSeparator + i +'0'
        }
        ...
        let enteroX = ctx.measureText('0').width/2
        let decNumbDist = eval(mainData.initValue) === 0 ? -enteroX : 0
        if (show.showAllValues.showAllValues === 'mostrar') {
          show.showAllValues.selectValuesToShow[i] === '1' && eval(mainData.initValue) !== 0 && ctx.fillText(eval(mainData.initValue), xPos - enteroX, yPos + scale.length*1.1)
        } else if (show.showAllValues.showAllValues === 'ocultar') {
          show.showAllValues.selectValuesToShow[i] !== '1' && eval(mainData.initValue) !== 0 && ctx.fillText(eval(mainData.initValue), xPos - enteroX, yPos + scale.length*1.1)
        } else {
          eval(mainData.initValue) !== 0 && ctx.fillText(eval(mainData.initValue), xPos - enteroX, yPos + scale.length*1.1)
        }
        //show.showAllValues.showAllValues ? eval(mainData.initValue) !== 0 && ctx.fillText(eval(mainData.initValue), xPos - enteroX, yPos + scale.length*1.1) : show.showAllValues.selectValuesToShow[i] === '1' && eval(mainData.initValue) !== 0 && ctx.fillText(eval(mainData.initValue), xPos - enteroX, yPos + scale.length*1.1)
        ctx.font = `${font.weight} ${font.size}px ${font.family}`
