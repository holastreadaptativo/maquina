...
export default class Overview extends Component {
	...
	drop(e) {
	    ...
	    if (drag.length > 1)
	    	respuestas('MOVE', { ...this.props, i:i, f:f })	    
	}
	handleWidth(e) {
		data.child(`${this.props.code}/answers/${e.target.id}`).update({ 
			width:e.target.value
		})
	}
	handleUpdate(params) {
		respuestas('UPDATE', { ...this.props, params:params, id:this.state.id })
		this.setState({ modal:false })
	}
	handleRemove(id) {
		if (confirm('¿Estas seguro de borrar la función?'))
			respuestas('REMOVE', { ...this.props, id:id })
	}
	getComponent() {
		let FX = null
		FUNCIONES.forEach(m => { 
